...
const _LifesPlayer = 3, _LifesBlock = 3,
...
var balas = [], balas2 = [], bloqueMarron = [], bloqueRojo = [], bloqueBlanco = [];
var iNave = new Image(),
...
function init(){
	...
	setTimeout(function(){
		...
		iNave.src          = 'assets/img/nave2.png';
...
function colision(){
	...
	for(i in bloqueBlanco){
		if(bloqueVida.chocar(bloqueBlanco[i])){
  			bloqueVida.x = random(canvas.width - 10);
  			bloqueVida.y = random(canvas.height - 10);
		}
	}
 ...
 // 			}
	...
	for(i in balas){
		for(j in bloqueBlanco){
 			if(balas[i].choqueVertical(bloqueBlanco[j]))
	  			balas[i].velY = -balas[i].velY;
	  		else if(balas[i].choqueHorizontal(bloqueBlanco[j]))
	  			balas[i].velX = -balas[i].velX;
		}
	}

// Bloques verticales		balas[i].velY = -balas[i].velY;
// Bloques horizontales		balas[i].velX = -balas[i].velX;
